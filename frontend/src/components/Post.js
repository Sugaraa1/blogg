...
function Post({ post, currentUser, onPostUpdate, showComments: initialShowComments = false }) {
  ...
  useEffect(() => {
    const recordView = async () => {
      try {
        ...
        const currentViews = post.repostedPost ? (post.repostedPost.views || []) : (post.views || []);
        if (!currentViews.includes(currentUser.id)) {
  ...
  const handleCommentLike = async (commentId, e) => {
    ...
    try {
      ...
      const postId = post.repostedPost ? post.repostedPost._id : post._id;
      const response = await axios.post(
        `${API_URL}/api/posts/${postId}/comment/${commentId}/like`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setComments(response.data.comments);
  ...
  const handleRetweet = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/posts/${postToRepost}/repost`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (onPostUpdate) onPostUpdate(response.data.post || response.data.repost);
    } catch (error) {
  ...
  const handleViewersClick = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/posts/${postId}/viewers`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const uniqueViewers = [];
      const seenIds = new Set();
      response.data.forEach(viewer => {
        if (!seenIds.has(viewer._id)) {
          seenIds.add(viewer._id);
  ...
  const handleDeletePost = async () => {
    if (!window.confirm('Энэ постыг устгахуу үнэхээр сайн уу?')) return;
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/api/posts/${post._id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleDeleteComment = async (commentId, e) => {
    e.stopPropagation();
    if (!window.confirm('Энэ сэтгэгдлийг устгахуу үнэхээр сайн уу?')) return;
    try {
      const token = localStorage.getItem('token');
      const postId = post.repostedPost ? post.repostedPost._id : post._id;
      await axios.delete(`${API_URL}/api/posts/${postId}/comment/${commentId}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div className="post">
      <Link to={`/profile/${displayPost.author?.username}`} className="post-avatar-link" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="post-body">
        ...
        <div className="post-header">
          <div className="post-author-info">
            <Link to={`/profile/${displayPost.author?.username}`} className="author-name" onClick={(e) => e.stopPropagation()}>
          ...
          {(isOwnPost || isOwnRepost) && (
            <button className="delete-post-btn" onClick={handleDeletePost} title={isOwnRepost ? "Share-ээ устгах" : "Постыг устгах"}>✕</button>
          )}
        </div>

        <div className="post-content"><p>{displayPost.content}</p></div>
        {displayPost.image && (
          <div className="post-image">
            <img src={displayPost.image.startsWith('http') ? displayPost.image : `${API_URL}${displayPost.image}`} alt="Post" />
          </div>
        )}

        <div className="post-actions">
          <button onClick={(e) => { e.stopPropagation(); setShowComments(!showComments); }} className="action-btn comment-btn">
            <span className="action-icon">💬</span>
            <span className="action-count">{comments.length}</span>
          </button>
          <button className={`action-btn retweet-btn ${isRetweeted ? 'retweeted' : ''}`} onClick={handleRetweet}>
            <span className="action-icon">🔄</span>
            <span className="action-count">{displayRetweets.length}</span>
          </button>
          <button onClick={handleLike} className={`action-btn like-btn ${isLiked ? 'liked' : ''}`}>
            <span className="action-icon">{isLiked ? '❤️' : '🤍'}</span>
            <span className="action-count">{likes.length}</span>
          </button>
          <button className="action-btn views-btn" onClick={handleViewersClick}>
          ...
          <div className="viewers-modal" onClick={(e) => e.stopPropagation()}>
            <div className="viewers-content">
              ...
              <div className="viewers-list">
                {viewers.length > 0 ? (
                  viewers.map((viewer) => (
                    <Link key={viewer._id} to={`/profile/${viewer.username}`} className="viewer-item" onClick={() => setShowViewers(false)}>
        ...
        {showComments && (
          <div className="comments-section" onClick={(e) => e.stopPropagation()}>
            <form onSubmit={handleComment} className="comment-form">
              ...
              <input type="text" placeholder="Хариулт бичих..." value={commentText} onChange={(e) => setCommentText(e.target.value)} />
              <button type="submit" disabled={!commentText.trim()}>Хариулах</button>
            </form>

            <div className="comments-list">
              {comments.map((comment) => (
                <div key={comment._id} className="comment">
                  <Link to={`/profile/${comment.user?.username}`} className="comment-avatar-link" onClick={(e) => e.stopPropagation()}>
                    <div className="comment-avatar-small">
                  ...
                  <div className="comment-body">
                    <div className="comment-author">
                      <Link to={`/profile/${comment.user?.username}`} onClick={(e) => e.stopPropagation()}>
                        <strong>{comment.user?.displayName}</strong>
                      </Link>
                      <span>@{comment.user?.username}</span>
                      {(comment.user?._id === currentUser.id || isOwnPost) && (
                        <button className="delete-comment-btn" onClick={(e) => handleDeleteComment(comment._id, e)}>✕</button>
                      )}
                    </div>
                    <p>{comment.text}</p>
                    <div className="comment-actions">
                      <button className="comment-action-btn" onClick={(e) => { e.stopPropagation(); setReplyingTo(replyingTo === comment._id ? null : comment._id); }}>
                        <span className="comment-action-icon">💬</span>
                        <span className="comment-action-count">{comment.replies?.length || 0}</span>
                      </button>
                      <button className={`comment-action-btn ${comment.likes?.includes(currentUser.id) ? 'liked' : ''}`} onClick={(e) => handleCommentLike(comment._id, e)}>
                        <span className="comment-action-icon">{comment.likes?.includes(currentUser.id) ? '❤️' : '🤍'}</span>
                        <span className="comment-action-count">{comment.likes?.length || 0}</span>
                      </button>
                    </div>
                    {replyingTo === comment._id && (
                      <form onSubmit={(e) => handleReply(comment._id, e)} className="reply-form" onClick={(e) => e.stopPropagation()}>
                        <input type="text" placeholder={`@${comment.user?.username}-д хариулах...`} value={replyText} onChange={(e) => setReplyText(e.target.value)} autoFocus />
                        <button type="submit" disabled={!replyText.trim()}>Илгээх</button>
                      </form>
                    )}
                    {comment.replies && comment.replies.length > 0 && (
                      <div className="replies-section">
                        <button className="toggle-replies-btn" onClick={(e) => { e.stopPropagation(); setShowReplies({ ...showReplies, [comment._id]: !showReplies[comment._id] }); }}>
                            ...
                            {comment.replies.map((reply, idx) => (
                              <div key={idx} className="reply-item">
                                ...
                                <div className="reply-content">
                                  <Link to={`/profile/${reply.user?.username}`}><strong>{reply.user?.displayName}</strong></Link>
